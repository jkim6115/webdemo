...
<h3>주석처리</h3>
	...
	<%-- 
		...
		1. jsp스크립트를 대신해서 편리하게 출력할 수 있도록 제공해주는 언어이다.
		2. ${ } 
	--%>
	...
	<%
		...
		// page영역에 p1이름으로 page값이 저장
		pageContext.setAttribute("p1", "page"); // 한 페이지
		
		// request영역에 p2이름으로 request값이 저장
		request.setAttribute("p2", "request"); // 한 번의 호출
		
		// session영역에 p3이름으로 session값이 저장
		session.setAttribute("p3", "session"); // 하나의 클라이언트
		
		// application영역에 p4이름으로 application값이 저장
	...
	<p>
	<!-- 
		속성값을 가져올 때 영역을 명시하지 않으면 page -> request -> session -> application순으로 검색
	 -->
	${ p3 }
	</p>
</body>
